...
@pytest.mark.parametrize(
    ...
    [
        ...
        for pre_trained in range(MIN_PRE_TRAINED_LEVEL, MAX_PRE_TRAINED_LEVEL)
    ],
)
def test_model_exists(env_name, pre_trained):
    agent, agent_info = opcc.get_policy(env_name, pre_trained)

    env = gym.make(env_name)
    obs = env.reset()
    obs = torch.tensor(obs).unsqueeze(0)
    action = agent(obs).data.cpu().numpy()[0].astype("float32")
